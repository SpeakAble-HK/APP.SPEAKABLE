...
export default function ProfileEditPage() {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error(err.message || '上傳頭像失敗');
    } finally {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (newPassword.length > 0 || confirmPassword.length > 0) {
      if (newPassword !== confirmPassword) { toast.error('新密碼不匹配'); return; }
      if (!PASSWORD_REGEX.test(newPassword)) { toast.error('密碼不符合要求'); return; }
    ...
    try {
      ...
      if (email !== user.email) {
        ...
        toast.info('確認電郵已發送至新地址，請驗證。');
      }
      if (newPassword.length > 0 && newPassword === confirmPassword) {
        const { error: signInError } = await supabase.auth.signInWithPassword({ email: user.email!, password: currentPassword });
        if (signInError) { toast.error('目前密碼不正確'); setSaving(false); return; }
  ...
  const initials = `${firstName?.[0] || ''}${lastName?.[0] || ''}`.toUpperCase() || '用';
    ...
    <div className="container mx-auto max-w-lg py-8 px-4">
      ...
      <Card className="card-shadow">
        <CardHeader className="text-center">
          <div className="flex justify-center mb-4">
            <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
              <Avatar className="h-20 w-20">
                {avatarUrl && <AvatarImage src={avatarUrl} alt="頭像" />}
          ...
          <CardTitle className="text-2xl">編輯個人資料</CardTitle>
          <CardDescription>更新你的帳號資料</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="profile-username">用戶名</Label>
              <Input id="profile-username" type="text" value={username} disabled readOnly className="opacity-85 cursor-not-allowed" />
              <p className="text-xs text-muted-foreground">用戶名在註冊後無法更改。</p>
            </div>
            <div className="space-y-2">
              <Label>出生日期</Label>
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !dateOfBirth && "text-muted-foreground")}>
                    ...
                    {dateOfBirth ? format(dateOfBirth, "PPP") : '選擇日期'}
                  </Button>
                </PopoverTrigger>
            ...
            <div className="space-y-2">
              <Label htmlFor="profile-email">電郵</Label>
              <Input id="profile-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
            </div>
            <div className="space-y-3 pt-2 border-t border-border">
              <p className="text-sm font-medium text-foreground pt-2">更改密碼</p>
              <div className="space-y-2">
                <Label htmlFor="profile-current-password">目前密碼</Label>
                <div className="relative">
                  <Input id="profile-current-password" type={showCurrentPassword ? "text" : "password"} value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} placeholder="輸入目前密碼" />
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input id="profile-new-password" type={showNewPassword ? "text" : "password"} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="輸入新密碼" />
                ...
                <p className="text-xs text-muted-foreground">{PASSWORD_HELPER_ZH}</p>
                {newPassword.length > 0 && !passwordValid && <p className="text-xs text-destructive">密碼不符合要求</p>}
              </div>
              <div className="space-y-2">
                <Label htmlFor="profile-confirm-password">確認新密碼</Label>
                <div className="relative">
                  <Input id="profile-confirm-password" type={showConfirmPassword ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="確認新密碼" />
                  <Button type="button" variant="ghost" size="icon" className="absolute right-1 top-1/2 -translate-y-1/2 h-7 w-7" onClick={() => setShowConfirmPassword(!showConfirmPassword)}>
                    {showConfirmPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                  </Button>
                </div>
                {confirmPassword.length > 0 && !passwordsMatch && <p className="text-xs text-destructive">密碼不匹配</p>}
              </div>
            </div>
